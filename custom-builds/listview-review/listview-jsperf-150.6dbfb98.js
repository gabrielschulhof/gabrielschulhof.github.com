...
$.widget150 = function( name, base, prototype ) {
	...
	// inheriting from
	basePrototype.options = $.widget150.extend( {}, basePrototype.options );
	...
	constructor.prototype = $.widget150.extend( basePrototype, {
	...
	if ( existingConstructor ) {
		$.each( existingConstructor._childConstructors, function( i, child ) {
			...
			$.widget150( childPrototype.namespace + "." + childPrototype.widgetName, constructor, child._proto );
		} );
	...
	$.widget150.bridge( name, constructor );

	return constructor;
};

$.widget150.extend = function( target ) {
			...
			if ( input[ inputIndex ].hasOwnProperty( key ) && value !== undefined ) {
				...
				if ( $.isPlainObject( value ) ) {
					target[ key ] = $.isPlainObject( target[ key ] ) ?
						$.widget150.extend( {}, target[ key ], value ) :
						// Don't extend strings, arrays, etc. with objects
						$.widget150.extend( {}, value );
...
$.widget150.bridge = function( name, object ) {
	...
	$.fn[ name ] = function( options ) {
		...
		if ( isMethodCall ) {
			...
			if ( args.length ) {
				options = $.widget150.extend.apply( null, [ options ].concat( args ) );
			}
...
$.Widget150 = function( /* options, element */ ) {};
$.Widget150._childConstructors = [];
...
$.Widget150.prototype = {
	...
	option: function( key, value ) {
		...
		if ( arguments.length === 0 ) {
			...
			return $.widget150.extend( {}, this.options );
		}
		...
		if ( typeof key === "string" ) {
			...
			key = parts.shift();
			if ( parts.length ) {
				curOption = options[ key ] = $.widget150.extend( {}, this.options[ key ] );
...
$.each( { show: "fadeIn", hide: "fadeOut" }, function( method, defaultEffect ) {
	$.Widget150.prototype[ "_" + method ] = function( element, options, callback ) {
		...
		if ( hasOptions && $.effects150 && $.effects150.effect[ effectName ] ) {
...
$.extend( $.mobile150, {
	...
	getAttribute: function( element, key ) {
		...
		if ( element && element.getAttribute ) {
			data = element.getAttribute( "data-" + $.mobile150.ns + key );
		}
	...
	nsNormalize: function( prop ) {
		return nsNormalizeDict[ prop ] ||
			( nsNormalizeDict[ prop ] = $.camelCase( $.mobile150.ns + prop ) );
	},
...
$.fn.jqmData150 = function( prop, value ) {
	var result;
	if ( typeof prop !== "undefined" ) {
		if ( prop ) {
			prop = $.mobile150.nsNormalize( prop );
		}
...
$.jqmData150 = function( elem, prop, value ) {
	var result;
	if ( typeof prop !== "undefined" ) {
		result = $.data( elem, prop ? $.mobile150.nsNormalize( prop ) : prop, value );
	}
	return result;
};

$.fn.jqmRemoveData150 = function( prop ) {
	return this.removeData( $.mobile150.nsNormalize( prop ) );
};

$.jqmRemoveData150 = function( elem, prop ) {
	return $.removeData( elem, $.mobile150.nsNormalize( prop ) );
};
...
return $.mobile150.widget = $.mobile150.widget || {};
...
// plugins
$.fn.extend( {
	scrollParent150: function( includeHidden ) {
	...
	labels150: function() {
		...
		if ( id ) {
			...
			selector = "label[for='" + $.ui150.escapeSelector( id ) + "']";
...
function visible( element ) {
	return $.expr.filters.visible150( element ) &&
...
$.extend( $.expr[ ":" ], {
	...
	focusable150: function( element ) {
		return focusable( element, $.attr( element, "tabindex" ) != null );
	},

	tabbable150: function( element ) {
...
// deprecated
$.ui150.ie = !!/msie [\w.]+/.exec( navigator.userAgent.toLowerCase() );

$.fn.extend( {
	disableSelection150: ( function() {
...
var compensateToolbars = function( page, desiredHeight ) {
	var pageParent = page.parent(),
		...
		noPadders = function() {
			var theElement = $( this ),
				widgetOptions = $.mobile150.toolbar && theElement.data( "mobile-toolbar" ) ?
					theElement.toolbar( "option" ) : {
						position: theElement.attr( "data-" + $.mobile150.ns + "position" ),
						updatePagePadding: ( theElement.attr( "data-" + $.mobile150.ns +
...
$.extend( $.mobile150, {
	...
	silentScroll: function( ypos ) {
		if ( $.type( ypos ) !== "number" ) {
			ypos = $.mobile150.defaultHomeScroll;
		}

		// prevent scrollstart and scrollstop events
		$.event.special.scrollstart150.enabled = false;

		setTimeout( function() {
			window.scrollTo( 0, ypos );
			$.mobile150.document.trigger( "silentscroll", { x: 0, y: ypos } );
		}, 20 );

		setTimeout( function() {
			$.event.special.scrollstart150.enabled = true;
		}, 150 );
	},
	...
	getClosestBaseUrl: function( ele ) {
		...
		var url = $( ele ).closest( ".ui-page" ).jqmData( "url" ),
			base = $.mobile150.path.documentBase.hrefNoHash;

		if ( !$.mobile150.base.dynamicBaseEnabled || !url || !$.mobile150.path.isPath( url ) ) {
			url = base;
		}

		return $.mobile150.path.makeUrlAbsolute( url, base );
	},
	removeActiveLinkClass: function( forceRemoval ) {
		if ( !!$.mobile150.activeClickedLink &&
				( !$.mobile150.activeClickedLink.closest( ".ui-page-active" ).length ||
				forceRemoval ) ) {

			$.mobile150.activeClickedLink.removeClass( "ui-button-active" );
		}
		$.mobile150.activeClickedLink = null;
	},
	...
	haveParents: function( elements, attr ) {
		if ( !$.mobile150.ignoreContentEnabled ) {
			return elements;
		}
		...
		for ( i = 0; i < count; i++ ) {
			...
			while ( e ) {
				c = e.getAttribute ? e.getAttribute( "data-" + $.mobile150.ns + attr ) : "";
	...
	getScreenHeight: function() {
		...
		return window.innerHeight || $.mobile150.window.height();
	},
	...
	loading: function() {
		...
		var loader = this.loading._widget || $( $.mobile150.loader.prototype.defaultHtml ).loader(),
...
$.addDependents150 = function( elem, newDependents ) {
	var $elem = $( elem ),
...
// plugins
$.fn.extend( {
	removeWithDependents150: function() {
		$.removeWithDependents( this );
	},

	addDependents150: function( newDependents ) {
	...
	getEncodedText150: function() {
		return $( "<a>" ).text( this.text() ).html();
	},

	// fluent helper function for the mobile namespaced equivalent
	jqmEnhanceable150: function() {
		return $.mobile150.enhanceable( this );
	},

	jqmHijackable150: function() {
		return $.mobile150.hijackable( this );
	}
} );

$.removeWithDependents150 = function( nativeElement ) {
	var element = $( nativeElement );

	( element.jqmData( "dependents" ) || $() ).remove();
	element.remove();
};
$.addDependents150 = function( nativeElement, newDependents ) {
	var element = $( nativeElement ),
...
$.mobile150.behaviors.addFirstLastClasses = {
...
return $.mobile150.behaviors.addFirstLastClasses;
...
var getAttribute = $.mobile150.getAttribute,
...
return $.widget150( "mobile.listview150", $.extend( {
	version: "@VERSION",
...
var plugin = {
		enhance150: function() {
			...
			for ( var i = 0; i < $.fn.enhance150.hooks.length; i++ ) {
				$.fn.enhance150.hooks[ i ].apply( this, arguments );
			}

			// Call the default enhancer function
			$.fn.enhance150.defaultFunction.apply( this, arguments );

			return this;
		}
	},
	getNamespace = function() {
		return $.fn.enhance150.ns || $.mobile150.ns || "";
	};

// Generate the init selector to be used by a widget
plugin.enhance150.initGenerator = function( prototype, ns ) {
...
plugin.enhance150.hooks = ( $.fn.enhance150 && $.fn.enhance150.hooks ) ? $.fn.enhance150.hooks : [];
plugin.enhance150._filter = $.fn.enhance150 ? $.fn.enhance150._filter || false : false;

// Default function
plugin.enhance150.defaultFunction = function() {
	...
	function crawlChildren( _childConstructors ) {
		...
		$.each( _childConstructors, function( index, constructor ) {
			var prototype = constructor.prototype,
				found = that.find(
					plugin.enhance150.initGenerator( prototype, getNamespace() )
				);

			if ( plugin.enhance150._filter ) {
				found = plugin.enhance150._filter( found );
			}
	...
	crawlChildren( $.Widget150._childConstructors );
};

// This is for backcompat remove in 1.6
plugin.enhanceWithin150 = function() {
	return this.children().enhance();
};

$.extend( $.fn, plugin );

$.extend( $.Widget150.prototype, {
