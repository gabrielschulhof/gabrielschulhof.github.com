...
(function( $, undefined ) {
	...
	$.widget( "mobile.textinput", $.mobile.textinput, {
		...
		_isIE8Textarea: function() {
			return ( "onpage" in this.element[ 0 ] );
		},

		// Synchronously (except in IE8, where it's asynch) fix the widget height if this widget's
		// parents are such that they show/hide content at runtime. We still need to check whether
		// the widget is actually visible in case it is contained inside multiple such containers.
		// For example: panel contains collapsible contains autogrow textinput. The panel may emit
		// "panelopen" indicating that its content has become visible, but the collapsible is still
		// collapsed, so the autogrow textarea is still not visible.
